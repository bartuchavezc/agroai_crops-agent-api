...
- **UNet/TorchServe settings:**
  ...
  - `UNET_LOCAL_MODEL_PATH`: Path to local model weights if not using TorchServe
  - `UNET_DEVICE`: Device to run the model on ('cuda', 'cpu')
...
UNET_LOCAL_MODEL_PATH=/path/to/local/unet/model.pt  # Optional
UNET_DEVICE=cpu  # cpu or cuda
