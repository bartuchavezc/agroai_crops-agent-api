...
class SegmenterService:
    ...
    def segment_image(self, image: Image.Image) -> SegmentationResult:
        ...
        try:
            ...
            # Convert numpy mask to torch tensor before overlaying
            mask_torch_tensor = torch.from_numpy(mask.astype(np.float32))
            overlay = self.overlay_mask(image, mask_torch_tensor)
    ...
    def overlay_mask(self, original_img: Image.Image, mask_tensor: torch.Tensor, color=(255, 0, 0), alpha=0.5):
        """
        ...
        Args:
            ...
            mask_tensor: torch.Tensor - The mask tensor (1, H, W) or (H, W)
